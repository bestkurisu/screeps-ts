import { collectCost, SHOW_BASE_CPU_COST, showCpuCost } from "./cpuLogger";
...
export const createApp = function (opt: Partial<CreateOptions> = {}) {
  ...
  const on = function (callbacks: AppLifecycleCallbacks): number {
    ...
    Object.keys(callbacks).forEach(type => {
      lifecycleCallbacks[type as keyof AppLifecycleCallbacks]?.push({
        id,
        callback: callbacks[type as keyof AppLifecycleCallbacks]!
      });
    });
    return id;
  };
